...
from pair.system_prompts_fixed import get_attacker_system_prompts
from pair.config import API_KEY_NAMES
import os 
...
def initialize_conversations(n_streams: int, goal: str, target_str: str, attacker_template_name: str, strategies: list[str]):
    batchsize = n_streams
    ...
    # Set system prompts
    system_prompts = get_attacker_system_prompts(goal, target_str, strategies)
    set_system_prompts(system_prompts, convs_list)
    # Compute per-conversation strategy assignment (cycled)
    strategy_assignments = [strategies[i % len(strategies)] for i in range(batchsize)] if strategies else []
    return convs_list, processed_response_list, system_prompts, strategy_assignments
